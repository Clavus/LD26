...
function TiledLevelData:processTiledData( data )
	...
	for k, tileset in ipairs(data.tilesets) do
		...
		newtileset.image = love.graphics.newImage(self._file_dir..tileset.image)
		...
		if (layer.type == "tilelayer") then
			...
			for i, id in ipairs(layer.data) do
				...
				if (id > 0) then
					...
					if (tset ~= nil) then
						local tx, ty, cn, rn = 0, 0, 0, 0
						local tid = (id - tilesetcache[tset.name].firstgid) -- index relative to tileset
						...
						local ay = 0  -- we need to adjust the y coord for different sized tiles. TileD scales them from the bottom left while we expect top left.
